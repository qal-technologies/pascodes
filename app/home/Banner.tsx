...
export default function Banner() {
  const services = React.useMemo(
    () => [
      { name: 'Web Apps', font: 'MomoTrust', color: 'green', size: 3.5 },
      { name: 'Softwares', font: 'Times New Roman', color: 'cyan', size: 3.5 },
      { name: 'Mobile Apps', font: 'SirinStencil', color: 'red', size: 3.5 },
      { name: 'UI Designs', font: 'SixtyFour', color: 'tomato', size: 3.0 },
      { name: 'UX Designs', font: 'Satisfy', color: 'yellow', size: 3.5},
      { name: 'Data Modeling', font: 'PoppinsBold', color: 'blue', size: 3.5 },
      { name: 'Coding Courses', font: 'mono', color: 'purple', size: 3.5 },
    ],
    [],
  );
  const wwa = [
    { name: 'Modern', color: 'brandNavy.900/30', icon: <BiTrendingUp /> },
    {
      name: 'Responsive',
      color: 'brandGreen.900/30',
      icon: <LuLayoutDashboard />,
    },
    {
      name: 'Interactive',
      color: 'brandGreen.400/10',
  ...
  return (
    <Flex
      ...
      <Box
        ...
        maxW='300px'
        maxH='300px'
        backgroundColor='brandGreen.700/50'
        filter='blur(15vh)'
        position='absolute'
        top='10vh'
        left='-12vh'
      ...
      <Box
        ...
        maxW='300px'
        maxH='300px'
        backgroundColor='brandNavy.500/40'
        filter='blur(15vh)'
        position='absolute'
        top='70vh'
        right='-12vh'
        ...
        <MotionText
          ...
          {mainService.name.split('').map((char, index) => (
            <MotionSpan
              key={index}
              fontSize={{
                base: `${mainService.size}rem`,
                lg: `${mainService.size + 1.05}rem`,
              }}
...
function ActionBtn({ type }: { type: 'build' | 'price' | string }) {
  ...
  const linkHref = isBuild ? '/build' : '#pricing-section';
  const width = isBuild ? '50%' : '50%';
  ...
  return (
    <>
      <Link
        ...
        _hover={{
          background: isBuild ? 'brandGreen.100' : 'white',
